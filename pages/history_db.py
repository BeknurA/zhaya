...
def show_history_db(lang_choice):
    ...
    if batches.empty:
        ...
        batch_options = {f"{row['product_type']} (ID: {row['batch_id']})": row['batch_id'] for index, row in batches.iterrows()}
        ...
        if 'production_batches' in df_db.columns and not df_db['production_batches'].isnull().all():
            try:
                ...
                if 'batch_product_type' in df_db.columns and 'batch_batch_id' in df_db.columns:
                    df_db['sample_name'] = df_db['batch_product_type'].astype(str) + ' - ' + df_db['batch_batch_id'].astype(str)
                else:
